...
| Wed Jan 28 | Green/Gurarie | Review 'Quantum Progress' exercise; Indexing vectors; knitr options | HW2 due, HW3 assigned |
...
This course uses Claude AI (via ESF's enterprise agreement with Anthropic) to analyze homework submissions and identify class-wide learning patterns. This approach aligns with SUNY's Faculty Advisory Council on Teaching and Technology ([FACT2](https://soar.suny.edu/bitstreams/77a56a43-fc22-43e6-9b9d-b7abe491351a/download) guidelines for responsible AI integration in higher education, which emphasize transparent use, human oversight, and student agency. We will use Sonnet 4.5 versus more advanced and computationally-intense models like Opus 4.5.
...
Key Protections:
 ...
 - You may opt out or request human-only review at any time
...
1. Download homework from Blackboard
2. Upload batch to Claude (ESF enterprise) for pattern analysis. Use the schell script to concantenate. Sonnet 4.5 will be faster, more cost effective, and better at this more defined task compared to Opus 4.5.
  + Prompt: "Analyze this batch of R homeworks for common patterns relative to these skill sub-objectives:
     - Valid R syntax. Effectiveness - does the code do what is asked of it?
     - Completeness. Does the code complete the task?
     - Logical approach. There are many ways to code some problems. Is the approach more-or-less straightforward?
     - Code efficiency. Is the code efficient? Are there unnecessary steps
     - Code reproducibility. (e.g., can the code be applied to a similar problem with minimal changes?)
     - Human readability. (e.g., the use of spaces, newlines; comments when helpful; avoid over-nesting and long lines of code)
    Analyze the ".Rmd" document from each student and provide:
     - aggregate class-level insights
     - a table listing issues for each student, the relevant skill sub-objective, the error or issue as completed by the student (copied), and the correct way to do it.
     - a horizontal barplot with bars for each skill sub-objective also showing lines for 80 and 90% class achievement."
3. Instructor review AI-generated report for: common errors, misconception patterns, skill or learning objective gaps
4. Independently grade each assignment using rubric
5. Use AI insights to inform: class discussion topics, additional examples needed
6. All grades determined through human judgment; AI = advisory input only
